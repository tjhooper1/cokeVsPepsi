...
const VoteContext = createContext<{state: State; dispatch: Dispatch} | undefined>(undefined);

function voteReducer(state: State, action: Action) {
	switch (action) {
		case 'pepsi':
			return { ...state, pepsiVotes: state.pepsiVotes + 1 };
		case 'coke':
			return { ...state, cokeVotes: state.cokeVotes + 1 };
		default:
			throw new Error();
	}
}
...
export function VoteProvider({ children }: {children: React.ReactNode}) {
	...
	return (
		<VoteContext.Provider value={{state, dispatch}}>{children}</VoteContext.Provider>
	);
}
